...
function initialize(config) {
  ...
  // Setup Views
  if (!config.theme_dir) {
    config.theme_dir = path.join(base_dir, 'themes');
  }
  ...
  app.set('views', path.join(base_dir, 'layouts'));
  app.set('view options', {
    layout: path.join(base_dir, 'layouts', config.site, config.theme_name)
  });
  ...
  if (config.theme_dir !== path.join(base_dir, 'themes')) {
